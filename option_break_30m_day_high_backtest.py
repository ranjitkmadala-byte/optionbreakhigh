...
# Market-aligned completed 30-minute candles:
# 09:15-10:15, 10:15-11:15, ... , 14:15-15:15
SESSION_START = dtime(9, 15)
LAST_COMPLETED_HOUR_END = dtime(15, 15)

# A "falling" 30m candle means close < open.
MIN_FALL_PCT = float(os.getenv("MIN_FALL_PCT", "0"))

# By default, the CE strike is the available strike nearest to the 30m SPOT HIGH.
...
DDL = """
CREATE TABLE IF NOT EXISTS public.option_break_30m_day_high_backtest (
...
CREATE INDEX IF NOT EXISTS idx_option_break_result
    ON public.option_break_30m_day_high_backtest
       (trading_date, trigger_reached, candle_start);

CREATE TABLE IF NOT EXISTS public.option_break_30m_day_high_summary (
    study_start DATE NOT NULL,
    study_end DATE NOT NULL,
    run_id UUID NOT NULL,
...
UPSERT = """
INSERT INTO public.option_break_30m_day_high_backtest (
...
def build_market_aligned_30m(rows):
    rows = [
        r for r in rows
    ...
    for r in rows:
        ...
        idx = mins // 30
        start = ts.replace(hour=9, minute=15, second=0, microsecond=0) + timedelta(minutes=30*idx)
...
def process_candle(sym, spot_key, day, candle, day_high):
    ...
    if candle["close"] >= candle["open"] or fall_pct < MIN_FALL_PCT:
        return None  # not a falling 30m candle

    # Strict rule: this 30m candle's HIGH must be the full-session DAY HIGH.
    ...
    if hour is None:
        raise ValueError("no option bars inside falling 30m candle")
    ...
    # Reverse-break trigger = reclaim/retest of the CE's own 30m high.
    trigger = hour["high"]
    z["trigger_level"] = trigger

    # Strict 30-minute reverse-break window:
    # after the falling 30m candle closes, the same CE must reclaim
    # that candle's CE high within the NEXT 30 minutes.
    trigger_deadline = min(
        candle["end"] + timedelta(minutes=30),
        candle["end"].replace(hour=15, minute=30, second=0, microsecond=0),
    )
    after = [
        r for r in option_rows
        if r["ts"] >= candle["end"]
        and r["ts"] < trigger_deadline
    ]
...
def main():
    ...
    for di, day in enumerate(days, 1):
        ...
        for i, u in enumerate(uni, 1):
            ...
            try:
                ...
                candles = build_market_aligned_30m(spot)
                ...
                for candle in candles:
                    ...
                    # Only the falling 30m candle that contains the full-session day high qualifies.
                    if abs(candle["high"] - day_high) > 1e-9:
                        continue
    ...
    FROM public.option_break_30m_day_high_backtest
    ...
    summary = {
        **safe,
        ...
        "failed": failed,
        "rule": {
            "spot_candle": "completed market-aligned 30m candle with close < open AND its high equals the full-session spot day high",
            "strike": "nearest available CE strike to the spot DAY HIGH / qualifying 30m candle HIGH",
            "reference": "CE high made during the same falling 30m candle",
            "success": "after the 30m candle ends, CE trades back to/reaches that reference high before EOD",
            "timing": "scenario may occur in any completed 30m candle from 09:15-15:15",
        },
    }

    qs = """
    INSERT INTO public.option_break_30m_day_high_summary (
